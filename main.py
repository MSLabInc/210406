...
def main():
    ...
    # 温度用キュー（現在：全て同じキューを使用,tc_readers_dict）
    """
    （以下、変数の中身の例）
    ...
    for str_port in config["Tc"].keys():
        ...
        save_file = f"output_{str_port[-1]}.csv"
    ...
    ssr_group_dict = {}
    """
    SSRの各々で、当面は、ループさせる。{210104}
    confis.jsonで指定した、Groupでの、ループにしたいとは思っている。
    """
    for i, target_pin in enumerate(config["SSR"]):   #ここでGroup別（当面SSRごと）にイテレーションLoopに。{201212}
    ...
    try:
        while True:
            ...
            ssr_group_dict[0].set_target_temp(500)   #60{210124A}, 200
